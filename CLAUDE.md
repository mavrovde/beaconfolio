...
| tooling | `scripts/verdict-heading-lib.sh` | **the ONE grammar that decides whether a body is a review verdict**, read by `pre-merge-gate.sh`, `audit_no_verdict_merges.sh` and `retro_metrics.sh` (v1.16.0 retro). All three had their own copy of "the first line states the verdict" and all three implemented it as *mentions* a marker, so the AUTHOR's own note outranked the reviewer's verdict. Measured over 229 merged PRs / 361 marker-bearing first lines: 354 are verdicts, 7 are not, and on #458 the author's *"the round-3 APPROVE covered `70a8cfb4`; … it needs a delta-confirm rather than standing"* passed BOTH the APPROVE test and the approval-covers-head test — for 14 minutes the gate would have merged on a sentence asking it not to. The grammar requires the first line to BEGIN with the marker, after at most a `Round N —` or `VERDICT:` prefix. Self-test `scripts/verdict-heading-lib.test.sh` (19 real headings, mutation contract 9/0/0), CI + the `vaudit` pre-push leg |
