...
| lint | `scripts/audit_no_verdict_merges.sh` | rule-13 detector for the half no hook can reach: finds merged PRs with NO canonical verdict (first-non-empty-line APPROVE/REQUEST CHANGES over reviews+comments). Run red-when-dirty by the scheduled `Verdict Audit` workflow since the 2026-09-14 cutover (#321/#355 merged verdict-less via the WEB UI, where PreToolUse hooks don't exist); self-test with live-path stub cases and a mutation contract in the pre-push gate + CI (#392). **Two limits, measured at v1.14.2 — do NOT read a green run as "the window was reviewed":** it tests **presence, not ordering**, so a verdict back-filled after the merge satisfies it (six were, and it reports that window clean), and its cron is **weekly** (`17 6 * * 1`), so detection latency is up to 7 days — v1.14.2's six violations all landed 8 hours after the week's only run. Both are fixed by **#409**. Meanwhile `pre-merge-gate.sh`'s bypass log (`$HOME/.claude/merge-gate-bypass.log`) is the companion instrument: an **empty** log beside a violation is positive evidence the merge never reached the CLI, which is stronger than the "provenance is unmeasurable" hedge the retros carried before |
