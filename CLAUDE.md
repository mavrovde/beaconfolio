...
| hook | `pre-merge-gate.sh` | PreToolUse Bash: refuses `gh pr merge` without an APPROVE verdict (read with the shared grammar in `scripts/verdict-heading-lib.sh`, so an author's note ABOUT a verdict is not one), with an APPROVE that predates the head, or with `Closes #NN` against unticked criteria (rule 13 enforced, not asked); a `PR_MERGE_GATE=0` bypass is allowed but never invisible — it appends to a local audit log and posts a PR comment (#392) |
...
| tooling | `scripts/verdict-heading-lib.sh` | **the ONE grammar that decides whether a body is a review verdict**, read by `pre-merge-gate.sh`, `audit_no_verdict_merges.sh` and `retro_metrics.sh` (v1.16.0 retro). All three had their own copy of "the first line states the verdict" and all three implemented it as *mentions* a marker, so the AUTHOR's own note outranked the reviewer's verdict. Measured over 229 merged PRs / 361 marker-bearing first lines: 354 state a verdict, 7 do not, and on #458 the author's *"the round-3 APPROVE covered `70a8cfb4`; … it needs a delta-confirm rather than standing"* passed BOTH the APPROVE test and the approval-covers-head test — for 14 minutes the gate would have merged on a sentence asking it not to. The grammar requires the first line to BEGIN with the marker, after at most a `Round N —` or `VERDICT:` prefix. Self-test `scripts/verdict-heading-lib.test.sh` (19 real headings, mutation contract 9/0/0), CI + the `vaudit` pre-push leg |
| tooling | `scripts/retro_metrics.sh` | the release window's retrospective figures as MEASUREMENTS, not hand counts: corpus bounded on the RELEASE PR's `mergedAt` (not the tag commit's date, which drops the release PR from its own release by one second), timestamps normalised to UTC, verdicts read with the shared grammar (`scripts/verdict-heading-lib.sh`) AND replayed at `mergedAt` so neither a back-filled review nor an author's fix report can be miscounted as a gate — prints corpus, median files/PR, canonical verdicts, mean rounds, round-1 approvals and rework share. Run it at `/retro` step 2 instead of re-deriving the jq. **BOTH bounds are release-PR merges since v1.15.1's retro**: pass the previous release's PR NUMBER and the lower bound is its `mergedAt`, because the tag's commit date lands one second before that merge and re-admitted the previous release's own PR into the next release's corpus (#436 was counted twice). A tag or ref still works and still bounds on the commit date, so earlier invocations reproduce; the printed window names which bound it used, and an all-digit argument that ALSO names a git commit is refused as ambiguous rather than guessed (`pr:`/`ref:` say which). **The listing is bounded SERVER-SIDE (`--search "merged:>=…"`) and guarded against truncation** since #452: it listed in DEFAULT order under a bare `--limit 100` and returned 7 of the v1.12.0 window's 10 PRs silently — a filled `RETRO_PR_LIMIT` (default 400) now exits 2 with "cannot measure", the same polarity as `audit_no_verdict_merges.sh`. **Self-test: `scripts/retro_metrics.test.sh` (17 cases + a 10/0/0 mutation contract), CI-only** — one mutant per refusal arm, each asserted on the script's OWN message because several arms share an exit code, and one argv-observed case pinning that the `pr list` call still carries `--search`, which is invisible in the output and is exactly what rotted. Class F ("a claim asserted rather than measured") has been the top or joint-top defect class in five consecutive retrospectives and its instances are overwhelmingly hand-counted figures in the retro docs themselves (v1.15.0 retro) |
| tooling | `scripts/verdict-audit-acknowledged.txt` | the ledger of rule-13 violations that ALREADY happened and cannot be repaired — read by `audit_no_verdict_merges.sh`. It exists because making that detector order-aware (#409) turned the cutover window from "clean" into seven real violations, and a permanently red alarm is a disabled alarm. Each PR is named individually WITH its shape and its retrospective write-up, so it silences exactly those merges: a NEW violation in the same window still goes red, a missing file acknowledges nothing, and acknowledged PRs are still PRINTED in the report. Adding a line is not a way to merge something unreviewed and tidy up afterwards — the list should shrink |
...
| lint | `scripts/check_changelog_merge.sh` | the `[Unreleased]` collision exists only in the MERGED result — this validates the REAL merge (`git merge-tree`, the machinery `gh pr merge` runs) of HEAD with `origin/main`: one `[Unreleased]`, no duplicate `### ` heading, no released heading deleted (#365) — except the narrow de-rotation of an UNSHIPPED release (newest heading, stub-only base `[Unreleased]`, every section line survives; the v1.14.3 revert) — no lost entry line; conflicts fail with rebase-first. Pre-push (`changelog` leg, selected by `CHANGELOG.md` diffs) + CI; mutation contract 9/0/0 (#391, v1.14.1 retro change A) |
| lint | `scripts/audit_no_verdict_merges.sh` | rule-13 detector for the half no hook can reach: finds merged PRs the gate did not actually gate (canonical verdict = a first non-empty line that STATES APPROVE/REQUEST CHANGES per `scripts/verdict-heading-lib.sh`, over reviews+comments, trusted association only). **ORDER-AWARE since #409** — a PR is judged on the NEWEST verdict posted at or before `mergedAt`, so a back-filled review no longer counts as a gate, and two shapes are reported distinctly: `NO-VERDICT` (nothing gated it) and `UNAPPROVED` (the newest pre-merge verdict was REQUEST CHANGES — the gate said no and the merge happened anyway; no prior release had a detector for this). Post-merge verdicts are surfaced as an informational `back-filled` note, never as a pass. Run **daily** by the scheduled `Verdict Audit` workflow (weekly meant up to 7 days of latency, and v1.14.2 opened and closed its violations inside one interval). The seven unrepairable historical violations are named ONE BY ONE in `scripts/verdict-audit-acknowledged.txt` — not amnestied by moving `--since`, so a NEW violation in the SAME window still goes red; a missing ledger acknowledges nothing, and the all-clear line says "N gated, M acknowledged" rather than claiming everything was gated. Self-test with live-path stub cases and a mutation contract in the pre-push gate + CI (#392/#409). Companion instrument: `pre-merge-gate.sh`'s bypass log (`$HOME/.claude/merge-gate-bypass.log`) — an **empty** log beside a violation is positive evidence the merge never reached the CLI |
...
| lint | `scripts/check_e2e_hydration_barrier.sh` | after a `page.goto` onto a PUBLIC (SSR) route, an e2e test must cross `waitForLoadState('networkidle')` before it types — otherwise `setUpControl`'s `writeValue` wipes the input as the control binds and the test fails 120s later at a disabled submit button, three steps from the cause. The axis is the NAVIGATION, not the directory: `blog-display.spec.ts` lives in `e2e/public/` but drives the CSR admin app, which has no pre-hydration window, so its fills are exempt. Pre-push (`e2ebarrier` leg) + CI; mutation contract 5/0/0. It exists because `contact-form.spec.ts` claimed in a comment that "Every other public spec uses this idiom" and that was FALSE — `cv.spec.ts` had four fills and no barrier, survived on timing luck, and was tipped red by #425's template rewrite |
